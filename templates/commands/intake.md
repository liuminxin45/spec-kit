---
description: Classify a CoreServicesLib request as migration, bugfix, or new-feature before specification.
---
...
- `migration`: moving existing Qt behavior into CoreServicesLib,
  DesktopShell, ProductNativePlugin/NativeBridge bridge, or frontend plugin surfaces.
...
5. Apply type-specific intake rules.
   ...
   - If the migration involves device identity, runtime state, RPC/N-API,
     ...
     bottom-layer internals, `NativeBridge` does not cache or calculate
     runtime truth, frontend operations use `node.uuid`, and events only
     trigger refresh from `CoreServicesLib`.
   - If the migration involves UI state, UI interactions, operation availability,
     ...
     `NativeBridge` is forwarding bridge only; runtime/status/permission/
     capability facts belong in `CoreServicesLib`; UI-display-specific
   ...
   - If the new feature involves device identity, runtime state, RPC/N-API,
     ...
     handles internal only, `NativeBridge` forwarding-only with no runtime
     cache, frontend operations using `node.uuid`, event-triggered refresh from
     `CoreServicesLib`, legacy API cleanup, and debug/test API isolation.
   - If the new feature involves UI state, UI interactions, operation
     ...
     `NativeBridge` forwarding bridge API, `CoreServicesLib` runtime/business
