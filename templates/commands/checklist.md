---
description: Generate a focused quality checklist for the active CoreServicesLib capability.
scripts:
...
- SDK, ProductNativePlugin/NativeBridge bridge, DesktopShell, or frontend plugin contracts.
...
- Identity / State / API Boundary: UUID decimal string is the only
  ...
  refresh but never replace `CoreServicesLib` as the truth source.
- UI/Biz/Libs layering: `NativeBridge` only forwards APIs; `CoreServicesLib`
...
- For required fact-layer evidence, verify `fact-pack.md`, latest log discovery
  from `C:\Windows\Temp\ExampleSdkLog\SDK_*.log` and
  `C:\Windows\Temp\NativeBridgeLog\NativeBridge_*.log`, plus chrome-devtools
