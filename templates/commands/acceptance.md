...
Load only `implementation-summary.md`, `validation.md`, and the active planning artifact needed to write user-facing checks.
...
3. Confirm validation status:
   - If `validation.md` is missing, return to `speckit.implement`.
   - If AI changed code and validation does not record PASS or a true external blocker, return to `speckit.implement`.
   - If `implementation-summary.md` is missing or does not describe the actual change, return to `speckit.implement`.
...
- UI/UX acceptance should cite available screenshots, DOM/CDP evidence, or owner-approved automation gaps.
