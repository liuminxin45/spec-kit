---
description: Create or update the CoreServicesLib team constitution and keep downstream templates aligned.
---
...
contract for CoreServicesLib, DesktopShell, ProductNativePlugin/NativeBridge bridge, SDK integration,
...
3. Interface compatibility first
   - Treat public SDK headers, ProductNativePlugin/NativeBridge bridge contracts, plugin APIs, frontend
...
5. Device and runtime truth
   - Do not fake SDK, device, transport, status, permission, or handle state.
...
6. UI display, Biz bridge, and Libs truth boundary
   - `NativeBridge` is an API forwarding bridge only. It must not implement
   ...
   - Non-UI-specific runtime facts, device/cache/handle/transport/permission
     data, and reusable business rules belong in `CoreServicesLib`.
   - Business-side code used only to display the current UI, such as UI
     ...
     `CoreServicesLib` facts obtained through the bridge.
...
7. Identity, state, and API single ownership
   ...
   - Events are refresh triggers only. After connect/disconnect/acquisition/
     runtime events, UI refreshes from `NativeBridge`, and `NativeBridge`
     forwards to `CoreServicesLib`; event payloads do not become a parallel truth
     store.
