---
description: Generate an actionable tasks.md for a CoreServicesLib capability from spec and plan artifacts.
scripts:
...
4. Organize tasks by delivery flow:
   ...
   - For UI state/UI interaction/operation availability work, include explicit
     tasks for `NativeBridge` forwarding-only API, `CoreServicesLib`
     runtime/permission/capability facts, and frontend plugin display
     composition. Do not put business logic in `NativeBridge`.
...
- UI element/order/visibility/enabled/action/permission changes must identify
  ...
  `CoreServicesLib`, and which `NativeBridge` bridge API forwards them.
...
- The final phase must include delivery closure tasks:
  ...
  - Use `commit-message` skill for commits and ask before staging/committing.
  - Run branch completion preflight only after retrospective/留痕 and commit, ask
    before cherry-pick, cherry-pick back to the base branch, 保留 spec branch,
    不删除 local spec branch by default, and 不 push.
...
- `NativeBridge` tasks must be forwarding-only. UI tasks may implement
...
- Acceptance/simplify/test-hardening/retrospective/commit/complete-branch
  closure tasks.
- Branch completion task, required user confirmation, 保留 spec branch policy,
  and any repositories that still need manual cherry-pick/completion handling.
- Required next stage: `speckit.analyze` / `$speckit-analyze`.
- Human review prompt:
  - Only ask for required human decisions: owner-approved gaps, external
    validation inputs, user acceptance, commit, or branch completion.
