...
candidates into long-lived locations such as `tools/spec-kit`, `.specify/memory`,
...
tools/spec-kit, TEAM-README, .specify/memory, or other team governance files.
...
7. Apply each approved candidate to the narrowest durable location:
   - `tools/spec-kit/templates/commands/*.md` for command behavior.
   - `tools/spec-kit/workflows/speckit/workflow.yml` for stage order or gate
     routing.
   - `tools/spec-kit/checklist-rules/*.yml` or checklist templates for
   ...
   - `tools/spec-kit/templates/ai/templates/validation-template.md` or
     `tools/spec-kit/templates/ai/templates/evidence-template.md` for reusable
     validation/evidence structure improvements.
   - Tests under `tools/spec-kit/tests/` when the lesson changes enforceable
     behavior.
...
- When the target is `tools/spec-kit`, add or update tests when the promoted
