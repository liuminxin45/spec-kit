---
description: Create or update a CoreServicesLib capability specification from a natural language request.
scripts:
  sh: scripts/bash/create-spec-branch.sh --json
  ps: scripts/powershell/create-spec-branch.ps1 -Json
---
...
even when it covers SDK, ProductNativePlugin/NativeBridge bridge, DesktopShell, frontend plugin,
...
6. The branch script must pass preflight for all workspace repositories before
   creating or switching any branch. If preflight fails, stop and report the
   repository-specific blocker instead of partially continuing.
...
8. Validate the generated checklist when the script exists:
   ...
   - Bash: `.specify/scripts/bash/validate-checklist.sh --feature-dir "<feature-dir>"`
...
- For UI state, UI interaction, operation availability, or device runtime UI:
  - State that `NativeBridge` is an API forwarding bridge only and must not
  ...
  - State that non-UI-specific runtime facts, permission/capability data, and
    reusable business rules belong in `CoreServicesLib`.
  - State that UI-display-specific composition, such as interaction surface structure,
    ...
    in the frontend plugin and must be based on `CoreServicesLib` facts obtained
    through the bridge.
...
- The spec branch is the workflow identity; final completion happens only after
  ...
  promote-lessons, commit, and complete-branch.
- Any commit or branch-state mutation requires explicit user confirmation.
