...
5. Identify issues:
   ...
   - `NativeBridge` implementing business logic instead of forwarding, UI
     label/string-based runtime/permission inference, fake device facts, or
     missing `CoreServicesLib` runtime/permission/capability facts.
   ...
   - `NativeBridge` caching device lists, connection/acquisition status, or
     runtime state.
   - Events replacing the truth source instead of triggering refresh from
     `CoreServicesLib` snapshot/runtime facts.
   ...
   - UI-interaction or operation-availability migration that lacks Qt source
     behavior coverage, or has coverage items without matching `CoreServicesLib`
     facts tasks, `NativeBridge` forwarding tasks, frontend display tasks,
