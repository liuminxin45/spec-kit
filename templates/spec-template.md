...
- **不涉及 / N/A 汇总**: [压缩列出已确认不涉及的 SDK/service/UI/runtime/external-system 等边界]
- **主要风险**: [兼容性、验证、数据、外部系统、编码、分支或生成物风险；无则写 N/A 和原因]
...
- **FR-LAYERING**: 若能力涉及 UI 状态、UI interaction、操作权限或运行态，系统必须明确
  三层边界：`forwarding bridge` 只能作为 API forwarding bridge；非 UI 专属的业务/运行时
  事实和可复用规则属于 `runtime/domain owner`；仅用于当前 UI 展示的结构、顺序、
  visible/enabled 组织和交互入口编排属于 frontend/presentation layer，并必须基于经 service layer 转发获得的
...
- **Extension / Runtime Contract**: [extension API, events, assets, runtime surface, or N/A]
- **Frontend State/UI Contract**: [UUID string identity, statusKey, operationPermissions,
  display state, or N/A]
- **UI Display Contract**: [frontend/presentation layer 如何基于 service layer 转发的 runtime/domain owner 事实组织
...
- **External System / Runtime Contract**: [external resource/cache/runtime state,
  or N/A]
...
涉及业务身份、实体列表、运行态、连接/采集状态、UI operation availability、RPC/API
或 public API 时必填；否则标记 `N/A` 并说明原因。

- **Canonical entity identity**: [format / N/A；跨 runtime facade、service layer、
  API/JSON/RPC、JS/UI 边界只允许这一种语义]
- **Internal native/external ids**: [handle/native id 仅存放在哪个底层实现内；
  不得跨到 runtime facade/service layer/UI，或 N/A]
- **Identity generation owner**: [owning domain/module / N/A；其它模块只使用]
...
- **Frontend operation identity**: [业务操作读取哪个 domain identity；UI node id/list index 仅为展示身份]
...
- **Naming boundary**: [业务身份 / UI node id / list index / cache key 等真实语义命名]

## 源行为覆盖清单

`migration` 涉及 UI interaction、operation availability 或运行态 UI 时必填；否则写 `N/A`
并说明原因。该清单必须覆盖影响 UI 显示/可用性的实体类型和状态维度，作为人工审核依据。
不强制使用固定表格格式，但必须包含：
...
推荐使用表格；复杂场景可以按实体类型分组、使用决策表、状态机说明、
fixture matrix 或按源函数分段的规则清单。
...
- **Quality Vision**: [quality-vision.md / N/A；UI/UX/copy/parity work requires
  baseline screenshot/design/source behavior or owner-approved N/A]
...
- **External-System Validation**: [target environment、simulation 或 N/A]
- **Downstream Check**: [consumer/regression check 或 N/A]
- **AI Self-Acceptance**: [PASS 前置验收要求；说明 log/runtime/API/test 证据入口]
