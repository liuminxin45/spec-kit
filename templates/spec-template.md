...
- **FR-LAYERING**: 若能力涉及 UI 状态、UI interaction、操作权限或设备运行态，系统必须明确
  三层边界：`NativeBridge` 只能作为 API forwarding bridge；非 UI 专属的业务/运行时
  事实和可复用规则属于 `CoreServicesLib`；仅用于当前 UI 展示的结构、顺序、
  visible/enabled 组织和交互入口编排属于 frontend plugin，并必须基于经 Biz 转发获得的
  `CoreServicesLib` 事实数据。
  不适用时必须说明 `N/A` 原因。
...
- **ProductNativePlugin / NativeBridge Bridge Contract**: [forwarded request/response fields; no business logic, or N/A]
- **DesktopShell / Plugin Contract**: [plugin API, events, assets, or N/A]
- **Frontend State/UI Contract**: [UUID string identity, statusKey, operationPermissions,
  display state, or N/A]
- **UI Display Contract**: [frontend plugin 如何基于 Biz 转发的 CoreServicesLib 事实组织
...
- **Device/Runtime Contract**: [ProtocolA/ProtocolB/ProtocolC/virtual device/cache/runtime
  state, or N/A]
...
- **Truth source**: [`CoreServicesLib` snapshot/runtime facts fields；Biz 不缓存、不计算]
...
- 对象/设备类型，例如 `ProtocolA`、`ProtocolB`、`ProtocolC`、`interface`。
...
- 目标契约来源：`CoreServicesLib` runtime/business facts、`NativeBridge` forwarding bridge、
...
| Target frontend/plugin | [path or N/A] | [ProductUIPlugin、ProductSuite plugin 或其他 UI 实现目标] |
