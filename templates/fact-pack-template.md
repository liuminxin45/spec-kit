...
- collect command:
- collected at:
- DeviceSdk log directory: `C:\Windows\Temp\ExampleSdkLog`
- DeviceSdk log pattern: `SDK_*.log`
- CoreServicesLib/Biz log directory: `C:\Windows\Temp\NativeBridgeLog`
...
- Correct Electron/DesktopShell target confirmed:
- Current route/page:
- DOM subtree:
- computed style:
- box metrics:
...
## 4. 日志事实
- Latest DeviceSdk log:
- Latest DeviceSdk LastWriteTime:
- Latest DeviceSdk key lines:
- Latest CoreServicesLib/Biz log:
- Latest CoreServicesLib/Biz LastWriteTime:
- Latest CoreServicesLib/Biz key lines:
- Whether logs were generated after DesktopShell exit:
...
## 9. 事实层规则
- Do not use MCP for log files; read `C:\Windows\Temp\ExampleSdkLog\SDK_*.log`
  and `C:\Windows\Temp\NativeBridgeLog\NativeBridge_*.log` directly.
