...
Public SDK headers、ProductNativePlugin/NativeBridge bridge contracts、DesktopShell plugin APIs、
...
`ProductNativePlugin/NativeBridge` 插件只能作为 API forwarding bridge：负责把
...
`CoreServicesLib`。仅服务于当前 UI 展示的业务侧组合代码，例如展示结构、条目顺序、
视觉禁用态组织、交互入口编排，应落在 frontend plugin，但必须基于 `CoreServicesLib`
经 `NativeBridge` 转发得到的运行时事实和能力数据；不得用 label/string 推断真实状态，
不得伪造 device/runtime/permission 事实，或长期缓存业务状态作为事实来源。

### 6. 接口与数据实现必须按职责拆分

当新增或扩展 `CoreServicesLib`、frontend plugin 或 `NativeBridge` API forwarding bridge
...
跨 `CoreServicesLib` facade、`NativeBridge`、N-API/JSON/RPC、JS 和 UI 边界的设备身份
...
`CoreServicesLib` snapshot/runtime facts。命名必须表达真实语义，例如 `uuid`、
...
模块和文件 ownership，例如 SDK headers、ProductNativePlugin/NativeBridge bridge implementation、
