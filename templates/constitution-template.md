...
Public SDK/API、native bridge contracts、host/plugin APIs、frontend state/events、
serialized fields、runtime status 和 operation permission semantics 都是兼容边界。
`native plugin`、`forwarding bridge`、`host application`、`runtime/domain owner` 等名称只是可选桌面宿主
/ plugin 场景示例，不是通用项目的默认架构要求。任何破坏兼容或改变行为的更新，都必须在
`spec.md` 和 `plan.md` 中说明影响、迁移方式和验证证据。
...
当项目存在 host/plugin/native 或 frontend/runtime 分层时，forwarding bridge 只能转发请求和响应；
不得实现业务规则、状态推导、权限/可用性判断、UI 行为计算或业务模型沉淀。非 UI 专属的业务能力、
runtime/cache/handle/transport/permission 事实和可复用规则必须落在对应 runtime/domain owner。
仅服务于当前 UI 展示的组合代码，例如展示结构、条目顺序、视觉禁用态组织、交互入口编排，
应落在 UI/frontend 层，但必须基于 runtime/domain owner 经 bridge 转发得到的事实和能力数据；
不得用 label/string 推断真实状态，不得伪造 runtime/permission 事实，或长期缓存业务状态作为事实来源。
在桌面宿主示例中，可将这些角色映射为 `runtime/domain owner`、`forwarding bridge` 和 frontend plugin。

### 6. 接口与数据实现必须按职责拆分

当新增或扩展 runtime/domain owner、frontend/UI 层或 API forwarding bridge
...
跨 runtime facade、bridge、N-API/JSON/RPC、JS 和 UI 边界的业务实体身份必须只有一个规范来源。
桌面设备类项目可使用十进制字符串 UUID；C++ 内部可以使用 `uint64_t uuid`，但不得在跨语言或
跨进程合同中引入 `deviceIndex`、`deviceId`、`handleId`、`virtualDeviceId` 等平行身份。
身份生成规则只能归属单一 owner，例如 `device::identity::generateUUID()`；manager、service
或 UI 层只能使用该身份，不得各自生成规则。native id、virtual id 和 handle 只能作为底层实现细节，
不得泄漏到 facade、bridge、service layer 或 UI。前端业务操作只能读取规范业务身份，例如 `node.uuid`；
`node.id` 是 UI tree node id，不是业务实体身份，不得用 `node.id`、`entityId`、`metadata.uuid`
等兜底操作设备或业务实体。

功能等价旧 API 不得与新 API 长期并存。旧的 forwarding path、facade、RPC 或 helper
应删除或明确迁移，不得形成调用方分裂。调试 API、测试 facade、临时 SDK 直通能力不得进入
生产 service layer exports；验证需求应放在测试、脚本或明确隔离的 debug-only tooling 中。事件只能
触发刷新，不能成为真相源；连接、断开、采集状态变化后仍必须通过 service layer 转发重新获取
runtime/domain owner snapshot/runtime facts。命名必须表达真实语义，例如 `uuid`、
...
模块和文件 ownership，例如 SDK/API headers、native bridge implementation、host/plugin code、
