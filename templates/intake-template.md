...
- **影响范围**: [主要模块/路径/仓库摘要]
- **不涉及 / N/A 汇总**: [压缩列出无关的 UI/service/runtime/external-system/design 等边界]
...
| Boundary type | [internal/public-api/ui-service-runtime/identity/runtime-status/permission/cross-repo] | [why] |
| Semantic risk | [low/medium/high] | [state/permission/external-system/API risk] |
...
- [ ] 单仓、通常 1-3 个文件。
- [ ] 内部实现改动，不涉及 public API、身份、权限/状态语义、外部系统、跨层或跨仓风险。
- [ ] 根因已有证据，不是猜测。
- [ ] 有本地可运行验证。
...
- [ ] 不涉及 public API、身份、权限/状态语义、外部系统、跨仓或选中的 gate pack。
...
**能力 / 缺陷 / 迁移目标**: [名称或摘要]
**Affected Area**: [SDK/API, integration/adapter layer, frontend/UI, tooling,
runtime/external-system, or mixed]
**目标模块**:
...
- [path/class/function/resource]: [作用]
...
- [build/smoke/unit/fixture/target-environment/manual comparison]
...
**疑似层级**: [SDK/API, integration/adapter layer, frontend/UI,
runtime/external-system, tooling, unknown]
**回归测试预期**: [unit/regression/fixture/smoke/manual or N/A]
**根因状态**: [known / suspected / unknown；known 必须说明证据]
**语义风险提示**: [是否涉及状态、权限、外部系统、身份、cache、UI operation availability、public API；不涉及写 N/A]

## 新增功能分流

当 `Task Type` 为 `new-feature` 时填写；否则标记为 `N/A`。

**为什么不是迁移**: [为什么这不是直接的既有行为迁移]
**新增能力意图**: [新增了什么价值或工作流]
**新增或变更契约**:
...
UI 状态、UI interaction、操作权限或运行态展示相关时必填；否则标记 `N/A`。

- **forwarding bridge 边界**: [forwarding bridge only / unknown / N/A；不得写业务逻辑]
- **runtime/domain owner 事实来源**: [runtime/permission/capability facts 来源，或 unknown / N/A]
...
当 `migration` 涉及 UI interaction、toolbar action、operation availability 或运行态 UI 时必填。
如果不相关，标记 `N/A` 并说明原因。该清单是进入 `plan` 前的审核输入，不能只写
“参考旧实现”。必须覆盖以下信息，但不强制使用固定表格格式：
...
推荐使用表格；当状态组合复杂时，可以按实体类型分组、使用决策表、状态机说明、
...
当 `migration` 和 `new-feature` 涉及 UI 行为、视觉状态、图标、
截图或设计资产时必填。仅当 UI 不相关时，才能标记 `N/A` 并说明原因。

| 目录类型 | Path | 适用类型 | 说明 |
|----------------|------|------------|-------|
| Original UI/source | [path or N/A] | migration | source files, resources, screenshots, or behavior references |
...
| Target UI/frontend | [path or N/A] | migration/new-feature | frontend、product surface 或其他 UI implementation target |
