...
- [ ] T004 如涉及 UI 状态、UI interaction、操作权限或 operation availability，确认 `NativeBridge` 仅做 API 转发；
  非 UI 专属的 runtime/permission/capability 事实来自 `CoreServicesLib`；仅用于 UI 展示的
...
- [ ] T012 [CS1] 如涉及 UI interaction 或 operation availability，在 `CoreServicesLib` 侧提供 runtime/permission/capability
  事实契约，在 `NativeBridge` 侧只提供转发接口，在 frontend plugin 侧实现 UI 展示
...
- [ ] TXXX 如相关，确认事件只触发刷新，刷新后重新从 `CoreServicesLib` 获取 snapshot/runtime facts。
- [ ] TXXX 如相关，确认 `build/`、`export/`、`plugin-out/` 等构建产物未参与 diff、接口判断或安装来源判断。
- [ ] TXXX 如相关，审核 Qt 源行为覆盖清单中的每个设备类型/状态组合都有实现、测试或
  owner-approved gap。
- [ ] TXXX 如相关，审核 `NativeBridge` 未实现业务逻辑；frontend plugin 未伪造或推断
  runtime/permission 事实；UI 展示组合只依赖 `CoreServicesLib` 事实数据和临时
  display state。
