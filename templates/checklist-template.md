...
- 质量门：生成后应运行 `.specify/scripts/powershell/validate-checklist.ps1`
  或 `.specify/scripts/bash/validate-checklist.sh`，并修复模板占位符、重复
...
- [ ] CHK008A 如涉及 UI 状态、UI interaction、operation availability 或操作权限，`spec.md`/`plan.md` 已明确
  `NativeBridge` 仅做 API forwarding bridge，不实现业务逻辑；非 UI 专属 runtime/
  permission/capability 事实来自 `CoreServicesLib`；仅用于 UI 展示的结构、顺序和
...
- [ ] CHK008B 如涉及 UI interaction/action availability，已覆盖 frontend-owned interaction/action id、顺序、可见性、
  可用性、action id、依赖的 `CoreServicesLib` permission/status/capability 来源和刷新时机。
...
- [ ] CHK010A 如相关，已确认 `NativeBridge` 未实现业务规则；frontend plugin 未用
  label/string 推断 runtime/permission 事实、未长期缓存 device/runtime/permission
  业务状态。

## 身份 / 状态 / API 边界

- [ ] CHK010D 跨 `CoreServicesLib` facade、`NativeBridge`、N-API/JSON/RPC、JS/UI 的设备身份
...
- [ ] CHK010H `NativeBridge` 未缓存设备列表、连接状态、采集状态或 runtime state；
  事件仅触发刷新，刷新后重新获取 `CoreServicesLib` snapshot/runtime facts。
...
- [ ] CHK014H Host-embedded frontend UI 如修改高度、flex、overflow、底部间距、详情栏/信息栏，
  已在真实 DesktopShell 宿主通过 CDP 记录 plugin root、shell、main panel、detail/footer panel、
...
- [ ] CHK018E DesktopShell CDP 验证已先列出 `/json/list` page targets，并记录
