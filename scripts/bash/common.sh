...
# Returns 0 when .specify/feature.json lists feature_directory that exists as a
# directory and matches the resolved active FEATURE_DIR.
...
read_feature_json_spec_branch() {
    ...
    local _branch=''
    if command -v jq >/dev/null 2>&1; then
        if ! _branch=$(jq -r '.spec_branch // empty' "$fj" 2>/dev/null); then
            _branch=''
        fi
    elif command -v python3 >/dev/null 2>&1; then
        if ! _branch=$(python3 -c "import json,sys; d=json.load(open(sys.argv[1])); v=d.get('spec_branch'); print(v if v else '')" "$fj" 2>/dev/null); then
            _branch=''
        fi
    else
        _branch=$( { grep -E '"spec_branch"[[:space:]]*:' "$fj" 2>/dev/null || true; } \
            | head -n 1 \
            | sed -E 's/^[^:]*:[[:space:]]*"([^"]*)".*$/\1/' )
    fi
...
get_feature_paths() {
    ...
    if [[ -n "${SPECIFY_FEATURE_DIRECTORY:-}" ]]; then
        ...
        local _fd _pinned_branch _effective_current _effective_pinned
        _fd=$(read_feature_json_feature_directory "$repo_root")
        _pinned_branch=$(read_feature_json_spec_branch "$repo_root")
        _effective_current=$(spec_kit_effective_branch_name "$current_branch")
        _effective_pinned=$(spec_kit_effective_branch_name "$_pinned_branch")
        if [[ -n "$_pinned_branch" && "$_effective_pinned" != "$_effective_current" ]]; then
            echo "ERROR: .specify/feature.json points to spec branch '$_pinned_branch' but current branch is '$current_branch'." >&2
            echo "Run /speckit.specify or create-spec-branch for the active spec before continuing." >&2
            return 1
        fi
        if [[ -n "$_fd" && -n "$_pinned_branch" ]]; then
            feature_dir="$_fd"
            # Normalize relative paths to absolute under repo root
            [[ "$feature_dir" != /* ]] && feature_dir="$repo_root/$feature_dir"
