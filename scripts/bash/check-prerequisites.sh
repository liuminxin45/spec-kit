...
EXAMPLES:
  ...
  ./check-prerequisites.sh --json --spec-only
    ...
    if $JSON_MODE; then
        if has_jq; then
            jq -cn \
                --arg feature_dir "$FEATURE_DIR" \
                --arg feature_spec "$FEATURE_SPEC" \
                '{FEATURE_DIR:$feature_dir,FEATURE_SPEC:$feature_spec,AVAILABLE_DOCS:[]}'
        else
            printf '{"FEATURE_DIR":"%s","FEATURE_SPEC":"%s","AVAILABLE_DOCS":[]}\n' \
