"""Base classes for Spec Kit integrations.
...
class IntegrationBase(ABC):
    ...
    * ``context_file``     — path (relative to project root) of the agent
                             context/instructions file (e.g. ``"AGENTS.md"``)
    """
    ...
    """Relative path to the agent context file (e.g. ``AGENTS.md``)."""
    ...
    def effective_invoke_separator(
        ...
        The default implementation ignores *parsed_options* and returns the
        class-level ``invoke_separator``.
        """
    ...
    def resolve_executable(command: str) -> str | None:
        """Resolve a command name to an executable path for subprocess calls.

        ``subprocess.run(..., shell=False)`` on Windows does not reliably apply
        shell aliases or every shim style that an interactive shell accepts.
        Resolve the first token up front so integrations can execute tools such
        as ``codex.cmd`` from Python even when PowerShell would have found a
        sibling ``codex.ps1`` first.
        """
        if not command:
            return None
    ...
    @classmethod
    def resolve_exec_args(cls, exec_args: list[str] | None) -> list[str] | None:
        """Return *exec_args* with argv[0] resolved, or ``None`` if missing."""
        if not exec_args:
            return None
        resolved = cls.resolve_executable(exec_args[0])
        if not resolved:
            return None
        return [resolved, *exec_args[1:]]
    ...
    def dispatch_command(
        ...
        *,
        ...
        resolved_args = self.resolve_exec_args(exec_args)
        if resolved_args is None:
            raise OSError(f"Executable not found: {exec_args[0]}")
        exec_args = resolved_args
    ...
    def shared_subskills_dir(self) -> Path | None:
        """Return path to bundled subskills for skills-capable integrations."""
        import inspect

        pkg_dir = Path(inspect.getfile(IntegrationBase)).resolve().parent.parent
        for candidate in [
            pkg_dir / "core_pack" / "subskills",
            pkg_dir.parent.parent / "templates" / "subskills",
        ]:
            if candidate.is_dir():
                return candidate
        return None
    ...
    def shared_templates_dir(self) -> Path | None:
        ...
        Contains ``vscode-settings.json``, ``spec-template.md``, layered
        ``ai/**`` source assets, and other shared templates. Checks
        ``core_pack/templates/`` then ``templates/``.
        """
        import inspect
    ...
    def agent_context_template(self) -> Path | None:
        """Return the AGENTS.md template for integrations that use AGENTS.md."""
        if self.context_file != "AGENTS.md":
            return None
        ...
        candidate = templates_dir / "agents-template.md"
    ...
    def list_subskill_templates(self) -> list[Path]:
        """Return sorted SKILL.md files from bundled subskill directories."""
        subskills_dir = self.shared_subskills_dir()
        if not subskills_dir or not subskills_dir.is_dir():
            return []
        return sorted(
            skill_file
            for skill_file in subskills_dir.glob("*/SKILL.md")
    ...
    def install_scripts(
        ...
        ``.specify/integrations/<key>/scripts/`` in the project. All copied
        files are recorded in *manifest*.
    ...
    def _build_context_section(plan_path: str = "") -> str:
        ...
        *plan_path* is the project-relative path to the current plan
        ...
        contains only the generic directive without a concrete path.  The
        directive is intentionally conditional so default agent context does
        not force every task to load plan.md.
        """
        lines = [
    ...
    def _upsert_context_file(
        ...
        *,
        ...
        body = section_body if section_body is not None else self._build_context_section(plan_path)
        section = (
        ...
        if ctx_path.exists():
            ...
            content = template.read_text(encoding="utf-8") if template else initial_content
            if content and context_file != "AGENTS.md":
                content = content.replace("# AGENTS.md", f"# {context_file}", 1)
    ...
    def upsert_context_section(
        ...
        """Create or update this integration's managed context section."""
        if not self.context_file:
            return None

        return self._upsert_context_file(
            project_root,
            self.context_file,
            plan_path,
            template=self.agent_context_template(),
        )
    ...
    def setup(
        ...
        **opts: Any,
        ...
        and call ``process_template()`` in their own loop.
        """
...
# ---------------------------------------------------------------------------
# SkillsIntegration — Codex skill-layout support
# ---------------------------------------------------------------------------
...
class SkillsIntegration(IntegrationBase):
    ...
    override ``options()`` to declare additional CLI flags.
    ...
    def internal_skills_dest(self, project_root: Path) -> Path | None:
        """Return the optional non-discovered skills output directory."""
        if not self.config:
            return None
        folder = self.config.get("folder")
        subdir = self.config.get("internal_skills_subdir")
        if not folder or not subdir:
            return None
        return project_root / folder / subdir

    def exposed_skill_names(self) -> set[str]:
        """Return skill names that should remain native-agent discoverable."""
        if not self.config:
            return set()
        names = self.config.get("exposed_skill_names")
        if not isinstance(names, list):
            return set()
        return {str(name) for name in names if str(name).strip()}

    def skill_output_dir(
        self,
        skill_name: str,
        skills_dir: Path,
        internal_skills_dir: Path | None,
        exposed_skill_names: set[str],
    ) -> Path:
        """Choose visible or internal output directory for a generated skill."""
        if exposed_skill_names and skill_name not in exposed_skill_names:
            return internal_skills_dir or skills_dir
        return skills_dir

    def decorate_core_skill_content(self, skill_name: str, content: str) -> str:
        """Add small integration-specific guidance to generated core skills."""
        exposed_skill_names = self.exposed_skill_names()
        folder = self.config.get("folder") if self.config else None
        internal_subdir = self.config.get("internal_skills_subdir") if self.config else None
        if (
            skill_name in exposed_skill_names
            and folder
            and internal_subdir
            and "## Internal Stage Loading" not in content
        ):
            internal_ref_dir = (Path(str(folder)) / str(internal_subdir)).as_posix()
            note = (
                "\n## Internal Stage Loading\n\n"
                "- This is the only Spec Kit skill exposed for native Codex discovery.\n"
                "- When a later Spec Kit stage is structurally required, load only that\n"
                f"  stage contract from `{internal_ref_dir}/speckit-<stage>/SKILL.md`.\n"
                "- Do not pre-load the internal skill directory; read one hidden skill at a\n"
                "  time after routing proves the stage is needed.\n"
            )
            return content.rstrip() + "\n" + note
        return content

    def remove_hidden_skills_from_exposed_dir(
        self,
        skills_dir: Path,
        ...
        """Remove stale generated skills that used to be exposed directly."""
        names_to_hide = generated_skill_names - exposed_skill_names
        names_to_hide.update(src.parent.name for src in self.list_subskill_templates())
        for skill_name in sorted(names_to_hide):
            skill_dir = (skills_dir / skill_name).resolve()
            try:
                skill_dir.relative_to(project_root_resolved)
            except ValueError as exc:
                raise ValueError(
                    f"Stale skill cleanup path {skill_dir} escapes "
                    f"project root {project_root_resolved}"
                ) from exc
    ...
    def post_process_skill_content(self, content: str) -> str:
        ...
        unchanged.  Subclasses may override for integration-specific behavior.
        """
        return content
    ...
    def setup(
        ...
        **opts: Any,
        ...
        internal_skills_dir = self.internal_skills_dest(project_root)
        if internal_skills_dir is not None:
            internal_skills_dir = internal_skills_dir.resolve()
            try:
                internal_skills_dir.relative_to(project_root_resolved)
            except ValueError as exc:
                raise ValueError(
                    f"Internal skills destination {internal_skills_dir} escapes "
                    f"project root {project_root_resolved}"
                ) from exc

        script_type = opts.get("script_type", "ps")
        arg_placeholder = (
        ...
        exposed_skill_names = self.exposed_skill_names()
        if exposed_skill_names:
            self.remove_hidden_skills_from_exposed_dir(
                skills_dir,
                set(generated_skill_names),
                exposed_skill_names,
        ...
        for src_file in templates:
            ...
            skill_content = (
                ...
                f"metadata:\n"
                f"  author: {_quote('spec-kit')}\n"
                f"  source: {_quote('templates/commands/' + src_file.name)}\n"
                f"---\n"
                f"{processed_body}"
            )
            skill_content = self.decorate_core_skill_content(
                skill_name,
                skill_content,
            )

            # Write speckit-<name>/SKILL.md
            target_skills_dir = self.skill_output_dir(
                skill_name,
                skills_dir,
                internal_skills_dir,
                exposed_skill_names,
        ...
        created.extend(
            self.install_subskills(
                project_root,
                manifest,
                internal_skills_dir or skills_dir,
                project_root_resolved,
            )
        )
    ...
    def install_subskills(
        ...
        """Install bundled hot-pluggable subskills without turning them into workflow steps."""
        created: list[Path] = []

        for src_file in self.list_subskill_templates():
            skill_dir_name = src_file.parent.name
            if not skill_dir_name:
                continue
            skill_file = (skills_dir / skill_dir_name / "SKILL.md").resolve()
            try:
                skill_file.relative_to(project_root_resolved)
            except ValueError as exc:
                raise ValueError(
                    f"Subskill destination {skill_file} escapes "
                    f"project root {project_root_resolved}"
                ) from exc

            raw = src_file.read_text(encoding="utf-8")
            dst = self.write_file_and_record(raw, skill_file, project_root, manifest)
            created.append(dst)
