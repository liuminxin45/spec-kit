...
_HOOK_COMMAND_NOTE = (
    ...
    "For example, `speckit.plan` -> `/speckit-plan`.\n"
)
...
class ClaudeIntegration(SkillsIntegration):
    ...
    def _agents_context_template(self) -> Path | None:
        """Return the shared canonical AGENTS.md template."""
    ...
    @staticmethod
    def _build_claude_context_section() -> str:
        """Build Claude's thin project-memory shim for Spec Kit."""
        return "\n".join(
            [
                "@AGENTS.md",
                "",
                "Spec Kit workflows are installed under `.claude/skills/`.",
                "Use `/speckit-specify`, `/speckit-plan`, `/speckit-tasks`,",
                "`/speckit-implement`, and related skills.",
            ]
        )
    ...
    def upsert_context_section(
        ...
        """Maintain AGENTS.md as canonical context and CLAUDE.md as a thin shim."""
        self._upsert_context_file(
            project_root,
            self.canonical_context_file,
            plan_path,
            template=self._agents_context_template(),
        )
        return self._upsert_context_file(
            ...
            section_body=self._build_claude_context_section(),
        )
