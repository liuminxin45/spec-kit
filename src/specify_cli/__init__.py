...
INIT_AI_CHOICES = {"codex", "claude"}
INIT_AI_MESSAGE = (
    "Spec Kit init defaults to Codex skills in this team distribution; "
    "pass --ai claude to initialize Claude Code instead."
)
...
def _install_shared_infra(
    ...
    Copies ``.specify/scripts/<variant>/``, ``.specify/templates/``,
    ``.specify/checklist-rules/``, and layered ``ai/**`` templates from the
    bundled core_pack or source checkout, where ``<variant>`` is ``bash``
    when *script_type* is ``"sh"`` and ``powershell`` when it is ``"ps"``.
...
SKILL_DESCRIPTIONS = {
    "intake": "Classify a CoreServicesLib request as migration, bugfix, or new-feature before specification.",
...
def init(
    ...
    ai: str = typer.Option(INIT_INTEGRATION, "--ai", help="AI coding workflow to initialize: codex or claude"),
    ...
    specify-cli package. The CoreServicesLib team distribution is intended to
    work without downloading templates from GitHub during initialization.

    This command will:
    1. Check that required tools are installed (git is optional)
    2. Initialize with Codex skills by default, or Claude Code skills with --ai claude
    3. Scaffold bundled project templates and workflows
    4. Initialize a fresh git repository (if not --no-git and no existing repo)
    5. Install the selected Spec Kit skills

    Examples:
        specify init my-project
        specify init my-project --no-git
        specify init --ignore-agent-tools my-project
        specify init --ai claude my-project
        specify init .                     # Initialize in current directory
        specify init --here
        specify init --here --force  # Skip confirmation when current directory not empty
        specify init my-project --preset healthcare-compliance  # With preset
    """

    show_banner()

    from .integrations import get_integration
    selected_ai = (ai or INIT_INTEGRATION).strip().lower()
    if selected_ai not in INIT_AI_CHOICES:
        console.print(
            f"[red]Error:[/red] Invalid --ai value '{ai}'. "
            f"Choose from: {', '.join(sorted(INIT_AI_CHOICES))}"
        )
        raise typer.Exit(1)
    ...
    steps_lines.append(f"{step_num}. Start the complete team workflow with your coding agent:")
    steps_lines.append(
        f"   Governance: run [cyan]{_display_cmd('constitution')}[/] only when bootstrapping or changing project principles."
    )

    workflow_steps = [
        ("intake", "Classify request, risk, and workflow weight"),
        ("specify", "Create the minimum intent artifact for the selected profile"),
        ("plan", "Create compact implementation slices"),
        ("implement", "Edit source, validate, and record progress"),
        ("acceptance", "Generate user acceptance steps and wait for confirmation"),
        ("retrospective", "Record Chinese-first workflow trace before commit"),
        ("commit", "Commit confirmed scope with team commit-message rules"),
        ("complete-branch", "Final cherry-pick to base branch, keep spec branch, and do not push"),
    ]
    for index, (command_name, description) in enumerate(workflow_steps, start=1):
        steps_lines.append(f"   {step_num}.{index} [cyan]{_display_cmd(command_name)}[/] - {description}")
    steps_lines.append(
        "   Profile-gated stages: standard-bugfix runs analyze before implement; "
        "full-sdd runs tasks -> analyze -> checklist before implement."
    )
    steps_lines.append(
        "   Other conditional stages: clarify, fact-layer, validation, simplify, "
        "test-hardening, and promote-lessons run when blockers, evidence, risk, "
        "or owner decisions require them."
    )
...
def preset_list():
    ...
    if not installed:
        ...
        console.print(
            "The CoreServicesLib team build has no remote preset catalog by default."
        )
...
def preset_info(
    ...
    # catalogs. This is the expected path for the CoreServicesLib team fork:
    # editable installs should show what lives under tools/spec-kit/presets.
    bundled_path = _locate_bundled_preset(preset_id)
    if bundled_path:
        try:
...
def extension_list(
    ...
    if not installed and not (available or all_extensions):
        ...
        console.print(
            "The CoreServicesLib team build has no remote extension catalog by default."
        )
        console.print("Use bundled workflows/templates or configure an internal catalog.")
        return
