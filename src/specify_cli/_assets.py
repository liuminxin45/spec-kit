...
def _source_checkout_available() -> bool:
    """Return True when this import is backed by the project source tree."""
...
def _locate_bundled_extension(extension_id: str) -> Path | None:
    ...
    Editable installs should prefer the source-checkout ``extensions/<id>/``
    directory so team-local changes are picked up immediately. Wheel installs
    still use ``core_pack``.
    """
    if not re.match(r'^[a-z0-9-]+$', extension_id):
        return None

    source_candidate = _repo_root() / "extensions" / extension_id
    if _source_checkout_available() and (source_candidate / "extension.yml").is_file():
    ...
    if (source_candidate / "extension.yml").is_file():
        return source_candidate
...
def _locate_bundled_workflow(workflow_id: str) -> Path | None:
    ...
    Editable installs should prefer the source-checkout ``workflows/<id>/``
    directory so team-local changes are picked up immediately. Wheel installs
    still use ``core_pack``.
    """
    ...
    source_candidate = _repo_root() / "workflows" / workflow_id
    if _source_checkout_available() and (source_candidate / "workflow.yml").is_file():
        return source_candidate
    ...
    # Source-checkout / editable install: look relative to repo root
    if (source_candidate / "workflow.yml").is_file():
        return source_candidate
...
def _locate_bundled_preset(preset_id: str) -> Path | None:
    ...
    Editable installs should prefer the source-checkout ``presets/<id>/``
    directory so team-local changes are picked up immediately. Wheel installs
    still use ``core_pack``.
    """
    ...
    source_candidate = _repo_root() / "presets" / preset_id
    if _source_checkout_available() and (source_candidate / "preset.yml").is_file():
        return source_candidate
    ...
    # Source-checkout / editable install: look relative to repo root
    if (source_candidate / "preset.yml").is_file():
        return source_candidate
