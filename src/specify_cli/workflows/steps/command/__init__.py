...
class CommandStep(StepBase):
    ...
    def _try_dispatch(
        ...
        # Check if the integration supports CLI dispatch and that Python can
        # execute its first argv token in this environment.
        test_args = impl.build_exec_args("test")
        if test_args is None:
            return None
        if impl.resolve_exec_args(test_args) is None:
            return None
