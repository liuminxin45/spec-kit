...
These files are repeatedly consumed by feature workflows. One-time init
templates such as ``agents-template.md`` and ``constitution-template.md`` stay
in the Spec Kit source/core pack and are read directly during initialization.
"""
...
INIT_ONLY_TEMPLATE_FILES = {
    ...
    "repository-map-template.md",
    "workspace-template.yml",
}
"""Source templates used to generate stable project assets once during init."""
...
def shared_checklist_rules_source(
    *,
    ...
    """Return the bundled/source shared checklist rules directory."""
    if core_pack and (core_pack / "checklist-rules").is_dir():
        return core_pack / "checklist-rules"
    return repo_root / "checklist-rules"


def shared_ai_templates_source(
    *,
    core_pack: Path | None,
    repo_root: Path,
) -> Path:
    """Return bundled/source AI rule, routing, knowledge, tool, and template docs."""
    templates_src = shared_templates_source(core_pack=core_pack, repo_root=repo_root)
    return templates_src / "ai"
...
def _is_runtime_template_file(src: Path) -> bool:
    return src.is_file() and src.name in RUNTIME_TEMPLATE_FILES


def _remove_stale_init_templates(
    project_path: Path,
    dest_templates: Path,
    manifest: IntegrationManifest,
) -> list[str]:
    """Remove init-only templates from the project runtime template directory."""
    removed: list[str] = []
    for name in sorted(INIT_ONLY_TEMPLATE_FILES):
        dst = dest_templates / name
        if not dst.exists():
            continue
        try:
...
def _remove_stale_managed_ai_assets(
    project_path: Path,
    manifest: IntegrationManifest,
    valid_ai_rels: set[str],
    modified: set[str],
    *,
    force: bool = False,
) -> tuple[list[str], list[str]]:
    """Remove managed ``ai/**`` files that no longer exist in the source pack.

    Deleted bundled AI docs should not linger forever in installed projects.
    Only files tracked by the shared manifest are considered. Customized files
    ...
    for rel in sorted(manifest.files):
        if not rel.startswith("ai/") or rel in valid_ai_rels:
            continue
...
def refresh_shared_templates(
    ...
    *,
    ...
    This covers both ``.specify/templates/*`` and the layered ``ai/**``
    templates installed by Spec Kit. Integration switches/upgrades should keep
    managed layered assets synchronized, while preserving user-customized files.
    """
    templates_src = shared_templates_source(core_pack=core_pack, repo_root=repo_root)
    ai_templates_src = shared_ai_templates_source(core_pack=core_pack, repo_root=repo_root)
    if not templates_src.is_dir() and not ai_templates_src.is_dir():
        return
    ...
    def _plan_template_update(src: Path, dst: Path) -> None:
        _ensure_safe_shared_destination(project_path, dst)
        rel = dst.relative_to(project_path).as_posix()
    ...
    if templates_src.is_dir():
        ...
        _remove_stale_init_templates(project_path, dest_templates, manifest)
        for src in templates_src.iterdir():
            if not _is_runtime_template_file(src):
                continue

            _plan_template_update(src, dest_templates / src.name)

    if ai_templates_src.is_dir():
        dest_ai = project_path / "ai"
        _ensure_safe_shared_directory(project_path, dest_ai)
        valid_ai_rels = {
            (Path("ai") / src.relative_to(ai_templates_src)).as_posix()
            for src in ai_templates_src.rglob("*")
            if src.is_file() and not src.name.startswith(".")
        }
        _, preserved_stale = _remove_stale_managed_ai_assets(
            project_path,
            manifest,
            valid_ai_rels,
            modified,
        ...
        skipped_files.extend(preserved_stale)
        for src in ai_templates_src.rglob("*"):
            if not src.is_file() or src.name.startswith("."):
                continue

            rel_path = src.relative_to(ai_templates_src)
            dst = dest_ai / rel_path
            _ensure_safe_shared_directory(project_path, dst.parent)
            _plan_template_update(src, dst)
    ...
    if skipped_files:
        console.print(
            f"[yellow]WARNING[/yellow]  {len(skipped_files)} modified or untracked shared template file(s) were not updated:"
        )
...
def install_shared_infra(
    ...
    *,
    ...
    if ai_templates_src.is_dir():
        dest_ai = project_path / "ai"
        if _ensure_or_bucket_dir(dest_ai):
            valid_ai_rels = {
                (Path("ai") / src_path.relative_to(ai_templates_src)).as_posix()
                for src_path in ai_templates_src.rglob("*")
            ...
            preserved_user_files.extend(preserved_stale)
            for src_path in ai_templates_src.rglob("*"):
                if not src_path.is_file() or src_path.name.startswith("."):
                    continue

                rel_path = src_path.relative_to(ai_templates_src)
                dst_path = dest_ai / rel_path
                rel = dst_path.relative_to(project_path).as_posix()
                if not _safe_dest_or_bucket(dst_path, rel, parent_must_exist=False):
                    continue
                write, bucket = _decide_overwrite(rel, dst_path)
    ...
    if checklist_rules_src.is_dir():
        dest_rules = project_path / ".specify" / "checklist-rules"
        if _ensure_or_bucket_dir(dest_rules):
            for src_path in checklist_rules_src.rglob("*"):
                if not src_path.is_file() or src_path.name.startswith("."):
                    continue

                rel_path = src_path.relative_to(checklist_rules_src)
                dst_path = dest_rules / rel_path
                rel = dst_path.relative_to(project_path).as_posix()
                if not _safe_dest_or_bucket(dst_path, rel, parent_must_exist=False):
                    continue
                write, bucket = _decide_overwrite(rel, dst_path)
    ...
    if skipped_files:
        console.print(
            f"[yellow]WARNING[/yellow]  {len(skipped_files)} shared infrastructure file(s) already exist and were not updated:"
        )
    ...
    if symlinked_files:
        console.print(
            f"[yellow]WARNING[/yellow]  Skipped {len(symlinked_files)} symlinked shared "
    ...
    if preserved_user_files:
        console.print(
            f"[yellow]WARNING[/yellow]  Preserved {len(preserved_user_files)} customized shared "
