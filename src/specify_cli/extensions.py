...
REINSTALL_COMMAND = r".\spec-kit\scripts\powershell\install.ps1"
...
class ExtensionManager:
    ...
    def _get_skills_dir(self) -> Optional[Path]:
        ...
        Reads ``.specify/init-options.json`` to determine whether Codex skills
        are enabled, then delegates to the module-level ``_get_skills_dir()``
        helper for the concrete path.
    ...
    def install_from_directory(
        ...
        Args:
            ...
            speckit_version: Current spec-kit version
            register_commands: If True, register commands with Codex skills
        ...
        # Register commands with Codex skills
        registered_commands = {}
        if register_commands:
            registrar = CommandRegistrar()
            # Register for the detected Codex skill target
    ...
    def remove(self, extension_id: str, keep_config: bool = False) -> bool:
        ...
        # Unregister commands from Codex skill targets
    ...
    def register_enabled_extensions_for_agent(self, agent_name: str) -> None:
        ...
        avoiding stale command directories when the active Codex target changes.
        """
        if not agent_name:
            return
...
class CommandRegistrar:
    """Handles registration of extension commands with Codex.
    ...
    and delegate to the shared registrar API.
    """
    ...
    def register_commands_for_all_agents(
        ...
        """Register extension commands for all detected Codex skill targets."""
        context_note = f"\n<!-- Extension: {manifest.id} -->\n<!-- Config: .specify/extensions/{manifest.id}/ -->\n"
...
class ExtensionCatalog:
    ...
    def get_active_catalogs(self) -> List[CatalogEntry]:
        ...
        # 4. Team build default: no remote extension catalogs.
        return []
...
class HookExecutor:
    ...
    def _render_hook_invocation(self, command: Any) -> str:
        """Render a Codex invocation string for a hook command."""
        if not isinstance(command, str):
            return ""
    ...
    def check_hooks_for_event(self, event_name: str) -> Dict[str, Any]:
        ...
        This method is designed to be called by Codex after core commands complete.
