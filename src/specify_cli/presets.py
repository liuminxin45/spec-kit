...
class PresetManager:
    ...
    def _register_commands(
        ...
        """Prepare preset command overrides for Codex skill reconciliation.

        Scans the preset's templates for type "command", reads each command
        file, and writes any composed command content under the installed
        preset directory so ``_register_skills()`` can consume the effective
        winner.
        ...
        Returns:
            Empty mapping. Codex-only Spec Kit does not write legacy command
            files; SKILL.md updates are tracked through ``registered_skills``.
        """
        command_templates = [
        ...
        for cmd in filtered:
            ...
            if strategy != "replace":
                ...
                if top_layer_is_ours:
                    ...
                    # Not the top layer; reconciliation will prepare the
                    # effective composed content after priority is settled.
                    continue
    ...
    def _reconcile_composed_commands(self, command_names: List[str]) -> None:
        ...
        for cmd_name in command_names:
            ...
            if not has_composition:
                # Pure replace is handled by _reconcile_skills().
                continue
            else:
                ...
                if composed is None:
                    ...
                    warnings.warn(
                        ...
                        f"Stale composed command files may remain.",
                        stacklevel=2,
                    )
                    continue
                ...
                if not registered:
                    ...
                    # shared .composed dir for later skill restoration.
        ...
        if non_preset_skills and skills_dir:
            ...
            for skill_name, cmd_name, top_layer in override_skills:
                ...
                try:
                    ...
                    # Apply integration post-processing when needed.
    ...
    def _get_skills_dir(self) -> Optional[Path]:
        ...
        are enabled for Codex, then delegates to the module-level
        ``_get_skills_dir()`` helper for the concrete path.
    ...
    def _register_skills(
        ...
        # Codex-only preset overrides update skills that already exist. The
        # hidden stage skills and exposed entry skill are created by init.
        ...
        for cmd_tmpl in filtered:
            ...
            # Only overwrite skills that already exist under skills_dir. If
            # both modern and legacy directories exist, update both.
    ...
    def install_from_directory(
        ...
        try:
            # Prepare composed command overrides for Codex skill registration.
            # Codex-only Spec Kit does not write legacy command files here.
    ...
    def remove(self, pack_id: str) -> bool:
        ...
        # Collect all command names before reconciliation. New Codex-only
        # installs keep registered_commands empty, so the manifest is the
        # authoritative source for primary command names and aliases.
                ...
                for tmpl in manifest.templates:
                    if tmpl.get("type") == "command":
                        ...
                        if isinstance(cmd_name, str):
                            removed_cmd_names.add(cmd_name)
                        for alias in tmpl.get("aliases", []):
                            if isinstance(alias, str):
                                removed_cmd_names.add(alias)
            except PresetValidationError:
                # Invalid manifest — rely on legacy registered_commands only.
                pass

        if registered_skills:
            self._unregister_skills(registered_skills, pack_dir)

        # Remove legacy command-skill artifacts recorded by older versions.
        if registered_commands:
            self._unregister_commands(registered_commands)
...
class PresetCatalog:
    ...
    def _make_request(self, url: str):
        ...
        Delegates to the GitHub-safe HTTP helper used by this Codex-only build.
        """
    ...
    def get_active_catalogs(self) -> List[PresetCatalogEntry]:
        ...
        # 4. Team build default: no remote preset catalogs.
        return []
...
class PresetResolver:
    ...
    def resolve(
        ...
        # (source-checkout / editable install).  In editable installs, prefer
        # the source tree even if a stale core_pack directory happens to exist.
        # This keeps team-local command/template edits live after uv editable
        # installation.
        ...
        if not source_checkout:
            # Source-checkout fallback for environments without core_pack.
            if template_type == "template":
                candidate = repo_root / "templates" / f"{template_name}.md"
            elif template_type == "command":
                candidate = repo_root / "templates" / "commands" / f"{template_name}.md"
