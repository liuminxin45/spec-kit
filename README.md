...
- **工作流闭环**：实现后先 `converge` 对齐承诺与交付证据，再进入人工验收；workflow run/status 支持 JSON。
...
`speckit` 默认链路在 implement 后增加 `speckit.converge`：AI 自验 PASS 以后，必须先写 `convergence.md`，检查 promised-vs-delivered 是否闭合；只有 `status: passed` 才能进入 `speckit.acceptance`。
...
2. 运行 `validate-spec-kit-version.ps1` 和仓库测试。
3. 提交升版改动。
4. 基于已验证提交创建 `v<x.y.z>` 标签。
