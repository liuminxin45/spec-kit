...
- **新任务开局保护**：`preflight-new-workflow.ps1` 在 intake 前阻断脏工作区、非 base 分支、未完成 `.specify` feature 和未解决 workflow run，避免覆盖用户现场。
...
- **可选 Workflow Hooks**：capability pack 可以声明阶段前/后的 `workflow-shell` 或 `workflow-agent-chain` hook；无 hook 时 workflow 行为保持不变，有 hook 时同步等待统一结果。
- **最终实施索引**：实现型 feature 会生成 `implementation-summary.md`，集中说明最终实际采用方案、代码/配置/脚本/文档/测试变更、机制差异、未做事项、验证验收结果和证据链接。
- **Root-Fix 决策门禁**：bugfix 计划必须比较 root fix、mitigation、containment 和 compatibility fallback，避免把清理、释放、重试、reset、fallback 或限制影响误描述成根治。
- **工作流闭环**：AI 自验、implementation summary、convergence、人工验收、retrospective、workflow observer、commit、post-commit self-check、rubric-score 和 complete-branch 串成固定闭环；workflow run/status 支持 JSON。
...
flowchart TD
    ...
    T --> D{"当前任务需要什么?"}
    D -->|"规格开发"| F["specs/<feature>/spec.md<br/>plan.md / progress.md<br/>implementation-summary.md"]
...
| `capabilities/index.yml` | 注册 pack 暴露的 skills、tools、scripts、commands、prompts、resources 和 hooks。 |
...
| `hooks/` | 可选 workflow hook 声明、脚本和 agent-chain manifest。`hooks/index.yml` 声明事件、`runner` 或 `chain_manifest`、timeout、failure policy 和工具依赖；安装/更新 pack 时会安装声明的 hook 工具依赖。 |
...
.specify/capabilities/hooks/<pack-id>/
```

## Workflow Hooks

Workflow hook 是 capability pack 的可选层，用来把外部工具或 Codex skill 链挂到 workflow stage 前后。它不属于默认核心流程：没有 `.specify/workflow-hooks.yml`，或者 registry 中没有匹配当前事件的 hook 时，workflow 行为、状态字段和 JSON/text 输出都保持不变。

事件命名固定为：

```text
workflow.<workflow-id>.<stage-id>.before
workflow.<workflow-id>.<stage-id>.after
```

例如 `workflow.speckit.commit.after` 表示在 `speckit` workflow 的 `commit` 阶段之后执行。v1 只触发 top-level workflow step 的 `before/after`，不触发嵌套控制流节点。

激活带 `type: workflow-shell` 的 hooks 后，compose 会生成 `.specify/workflow-hooks.yml`。Workflow engine 会调用 `invoke-workflow-hooks.ps1` 同步执行并等待结果，runner 会把脚本退出码、结果文件或 stdout JSON 归一化为统一结果：
...
只有 `auto_continue=true` 时 workflow 才进入下一阶段；`blocked`、`failed`、`requires_rework` 或任何 `auto_continue=false` 的结果都会让 workflow 暂停，并把 hook summary/artifacts 记录到状态中。可用 `.specify/workflow-hooks.local.yml` 在本机临时禁用全部 hooks、指定事件、指定 hook id 或指定 pack id；禁用后不会写入 hook 状态。

`type: workflow-agent-chain` 适合表达“一个 hook 节点内串行执行多个 Codex skill”的场景。Pack 的 `hooks/index.yml` 声明 `chain_manifest`，manifest 里写 `steps`，每个 step 指定 `skill`、可选 `run_if_previous_status`、`timeout_seconds`。Workflow engine 会按顺序执行 skill，把 `previous_result` 和 `previous_results` 传给下一步；第一个 `auto_continue=false`、`blocked`、`failed` 或 `requires_rework` 会短路链路并暂停 workflow。`commit.after` 链如果留下 tracked dirty files，会被归一化为 `requires_rework`，不能在提交后悄悄改代码再继续。

Hook 工具依赖随 pack 声明和安装。支持的安装方式包括 `pack-local-script`、`npm`、`github-release` 和 `manual`；安装记录会写入 `.specify/tools/records/`，包含工具 id、固定版本、安装方式、resolved command、verify command 和锁定信息。卸载 pack 会移除对应 hook 发布目录并重新生成 registry，同时只清理不再被任何 active pack 引用的 hook 工具版本。
...
生成的 pack 只记录工具 id、固定版本、安装方式、校验命令、hook 事件和 wrapper 脚本；不会打包用户的 LLM API key、机器路径或全局工具安装产物。以 `open-code-review` 为例，Spec Kit 只负责把工具以 hook 形式接入、安装/验证固定版本并归一化结果；工具自身需要的模型配置仍由用户按该工具文档在本机或 CI 中配置。
...
| --- | --- | --- |
| `micro-fix` | 单仓、低风险、root cause 明确、可本地验证的 1-3 文件修复。 | `micro-fix.md` 或 `progress.md`，实现后看 `implementation-summary.md` |
| `standard-bugfix-lite` | 低/中风险标准修复的轻量路径。 | `workpack.md`，实现后看 `implementation-summary.md` |
| `standard-bugfix` | 需要 `spec.md` + `plan.md`，但不一定需要独立 `tasks.md` 的普通行为修复。 | `spec.md`、`plan.md`，实现后看 `implementation-summary.md` |
| `full-sdd` | public API、架构/协议、跨仓、迁移、真实设备或 host/plugin/native 交付链。 | `spec.md`、`plan.md`、`tasks.md`、`implementation-summary.md` 和按需设计 artifact |
...
```text
intake -> specify -> plan -> implement -> implementation-summary -> converge -> acceptance
-> human-acceptance -> retrospective -> workflow-observer -> commit
-> post-commit-self-check -> rubric-score -> complete-branch
```

后续查看某个 feature “最终真正实施了什么”时，优先看
`specs/<feature>/implementation-summary.md`。它是摘要索引，不复制全部日志：
只记录最终方案、实际变更清单、机制变化、plan/spec 差异、未实施事项、
验证/验收摘要、残留风险和证据文档链接。
