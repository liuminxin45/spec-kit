...
- **可选 Workflow Hooks**：capability pack 可以声明阶段前/后的确定性 hook；无 hook 时 workflow 行为保持不变，有 hook 时同步等待统一结果。
...
| `hooks/` | 可选 workflow hook 声明和脚本。`hooks/index.yml` 声明事件、runner、timeout、failure policy 和工具依赖；安装/更新 pack 时会安装声明的 hook 工具依赖。 |
...
Workflow hook 是 capability pack 的可选层，用来把外部确定性工具挂到 workflow stage 前后。它不属于默认核心流程：没有 `.specify/workflow-hooks.yml`，或者 registry 中没有匹配当前事件的 hook 时，workflow 行为、状态字段和 JSON/text 输出都保持不变。
