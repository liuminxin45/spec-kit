...
当前对齐基线为 `upstream_baseline: github/spec-kit@v0.12.5`；本仓库版本独立演进，当前本地版本线为 `0.10.1`，不冒用官方 `0.12.x` 版本号。
...
| 闭环 | 实现后由团队自行收口 | `implementation-summary.md`、convergence、acceptance、retrospective、workflow observer、commit、post-commit self-check、rubric-score | 交付事实更容易追溯 |
...
flowchart LR
    ...
    subgraph ThisRepo["本仓库 spec-kit"]
      ...
      T3 --> T4["Implement"]
      T4 --> T5["AI self-acceptance<br/>tests / logs / browser / CDP"]
      T5 --> T6["Converge"]
      T6 --> T7["Human acceptance"]
      T7 --> T8["Retrospective / commit / self-check"]
    end
...
| Profile | 适用场景 | 主要产物 |
| --- | --- | --- |
| `micro-fix` | 1-3 个文件、低风险、根因明确的小修复 | `micro-fix.md` 或 `progress.md` |
...
```text
intake -> specify -> plan -> implement -> AI self-acceptance -> converge
-> acceptance -> human-acceptance -> retrospective -> workflow-observer
...
提升 retrospective 中 approved 知识候选：
...
不会作为普通交付默认动作。默认是本地分支、人工验收、自动提交、post-commit self-check、rubric-score，最后经人工确认后本地 complete-branch。push 在默认流程之外，应优先 PR-first；如确需例外直推，必须先通过 `preflight-push` 并取得明确人工批准。
